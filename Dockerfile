...
FROM cgr.dev/chainguard/node:latest-dev@sha256:2a2df3a1f79cfe63317e3e6e2b7394cac434648fdfc82e4f702d0e6dc4d9a852
...
&& MISE_VERSION=2026.9.5 MISE_INSTALL_PATH=/usr/local/bin/mise sh /tmp/mise-install.sh \
...
&& ln -s "$(mise exec uv@0.12.13 -- which uv)" /usr/local/bin/uv \
&& ln -s "$(mise exec uv@0.12.13 -- which uvx)" /usr/local/bin/uvx
