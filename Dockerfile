...
&& MISE_VERSION=2026.8.5 MISE_INSTALL_PATH=/usr/local/bin/mise sh /tmp/mise-install.sh \
...
&& ln -s "$(mise exec uv@0.12.3 -- which uv)" /usr/local/bin/uv \
&& ln -s "$(mise exec uv@0.12.3 -- which uvx)" /usr/local/bin/uvx
